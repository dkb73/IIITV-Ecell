...
const WingCard = ({ href, title, description }: { href: string; title: string; description: string }) => {
  return (
    <Link href={href} className="group bg-background rounded-lg shadow-md p-6 hover:shadow-xl transition-all duration-300">
      <div>
        <div className="text-center">
          <h3 className="text-2xl font-bold relative inline-block">
            <span className="bg-gradient-to-r from-primary to-primary bg-[length:0px_2px] group-hover:bg-[length:100%_2px] bg-left-bottom bg-no-repeat transition-all duration-500">
...
const Wings = () => {
  const wingsData = [
    {
      href: "/wings-details/#irp",
      title: "IRP",
      description: "Industrial Resource Planning (IRP) wing of the Entrepreneurship Cell focuses on bridging the gap between academia and industry."
    },
    {
      href: "/wings-details/#ssip",
      title: "SSIP",
      description: "The Student Startup and Innovation Policy (SSIP) is our initiative to support student entrepreneurs. We offer funding, mentorship, and networking opportunities."
    },
    {
      href: "/wings-details/#incubation",
      title: "Incubation",
      description: "Our incubation program provides a nurturing environment for student startups. We offer workspace, funding, and access to industry experts."
    },
    {
      href: "/wings-details/#skilldevelopment",
      title: "Skill Development",
      description: "We organize workshops, training sessions, and resources to help students develop the skills needed to become successful entrepreneurs."
    }
  ];
      ...
      <div className="px-4 md:px-8 lg:px-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {wingsData.map((wing, index) => (
            <WingCard key={index} href={wing.href} title={wing.title} description={wing.description} />
          ))}
        </div>
      </div>
    </section>
