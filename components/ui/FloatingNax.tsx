...
export const FloatingNav = ({
  ...
  return (
    <>
      {/* Mobile Menu Button Row */}
      <div className="fixed top-0 left-0 right-0 z-1000 flex justify-end items-center bg-[#2d4073] p-4 md:hidden">
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            exit={{ x: '100%' }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            className={cn("fixed top-0 right-0 h-full w-3/4 bg-[#2d4073] shadow-lg z-50", className)}
          >
            <div className="p-4 flex justify-between items-center">
              <Link href="/#home">
                <Image
            ...
            <div className="p-4">
              {navItems.map((item, index) => (
                <div key={index} className="relative">
                  <div className="flex items-center">
                    <Link href={item.link} className="block py-2 px-4 text-lg text-white hover:bg-[#3d5393] transition-colors duration-200">
                      {item.icon && <span className="mr-2">{item.icon}</span>}
                      {item.name}
                    </Link>
                    {item.subItems && (
                      <IconButton onClick={toggleMobileDropdown} className="text-white">
                        <ArrowDropDownIcon className="text-white" />
                      </IconButton>
                    )}
                  </div>
                  {item.subItems && mobileDropdownOpen && (
                    <div className="mt-2 w-full bg-[#3d5393] rounded-md z-50">
                      {item.subItems.map((subItem, subIdx) => (
                        <Link
                          key={`subLink=${subIdx}`}
                          href={subItem.link}
                          className="block px-4 py-2 text-white hover:bg-[#4d63a3] transition-colors duration-200"
                        >
                          {subItem.name}
                        </Link>
                      ))}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={cn(
            "hidden md:flex w-full fixed top-0 inset-x-0 mx-auto border-b border-[#3d5393] bg-[#2d4073] shadow-lg z-[5000] px-10 py-5 items-center justify-between",
            className
          )}
        >
          <Link href="/#home">
          ...
          <div className="flex space-x-4 ml-auto">
            {navItems.map((navItem: any, idx: number) => (
              <div key={`link=${idx}`} className="relative flex items-center">
                ...
                {navItem.subItems && (
                  <>
                    <IconButton onClick={toggleDropdown} className="text-white">
                      <ArrowDropDownIcon className="text-white" />
                    </IconButton>
                    {dropdownOpen && (
                      <div className="absolute top-full left-0 mt-2 w-48 bg-[#2d4073] shadow-lg rounded-md z-50">
                        {navItem.subItems.map((subItem: { name: string; link: string }, subIdx: number) => (
                          <Link
                            key={`subLink=${subIdx}`}
                            href={subItem.link}
                            className="block px-4 py-2 text-white hover:bg-[#3d5393] transition-colors duration-200"
                          >
                            {subItem.name}
                          </Link>
                        ))}
